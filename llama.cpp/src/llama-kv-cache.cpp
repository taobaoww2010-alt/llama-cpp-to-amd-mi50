...
llama_kv_cache::llama_kv_cache(
    ...
    const char * use_paged_env = getenv("LLAMA_USE_PAGED_KV");
    if (use_paged_env && atoi(use_paged_env) == 1) {
        // Paged mode not supported with multi-device tensor splitting yet
        if (model.n_devices() > 1) {
            fprintf(stderr, "=== PagedKVCache: Disabled for multi-device (%zu devices) ===\n", model.n_devices());
        } else {
            use_paged = true;

            ggml_type type_k_q = type_k;
            ggml_type type_v_q = type_v;

            const char* kv_quant_env = getenv("LLAMA_KV_QUANT");
            if (kv_quant_env) {
                int qtype = atoi(kv_quant_env);
                if (qtype == 8) {
                    type_k_q = GGML_TYPE_Q8_0;
                    type_v_q = GGML_TYPE_Q8_0;
                    fprintf(stderr, "=== PagedKVCache: Using Q8_0 quantization ===\n");
                }
            }

            paged_cache.init(model, type_k_q, type_v_q, offload, kv_size, n_seq_max, nullptr);
        }
    }
...
llama_kv_cache::slot_info llama_kv_cache::find_slot(const llama_ubatch & ubatch, bool cont) const {
    ...
    // Paged mode: use PagedKVCache for position tracking
    // Only for single sequence - multi-sequence needs original path
    if (use_paged && ubatch.n_seqs_unq == 1) {
        // Delegate slot allocation to PagedKVCache for the single sequence
        const auto seq_id = ubatch.seq_id_unq[0];
        int result = paged_cache.find_slot(seq_id, ubatch.n_tokens);
        if (result < 0) {
            return { };
        }

        // Build slot_info for single sequence
        slot_info res;
        res.resize(1);
        ...
        const auto* table = paged_cache.get_seq_table(seq_id);
        if (!table) {
            return { };
        }

        // Convert this sequence's block positions to local indices
        res.idxs[0].reserve(ubatch.n_tokens);
        int local_idx = 0;
        for (const auto& mapping : table->get_blocks()) {
            const int n = std::min(mapping.n_tokens, (int)ubatch.n_tokens - local_idx);
            for (int i = 0; i < n; ++i) {
                res.idxs[0].push_back(local_idx);
                ++local_idx;
            }
            if (local_idx >= (int)ubatch.n_tokens) break;
        }

        return res;
    }

    // For multi-sequence or non-paged mode, use original path
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    // Paged mode: use BlockPool gather
    // Only use paged gather for single-stream (single sequence) - multi-stream needs fallback
    if (use_paged && sinfo.n_stream() == 1 && !sinfo.strm.empty()) {
        const uint32_t seq_id = sinfo.strm[0];
        ggml_tensor* k_from_pool = paged_cache.gather_k_view(ctx, nullptr, seq_id, n_kv);
        if (k_from_pool) {
            return k_from_pool;
        }
        // Fallback: if gather fails, try original path
    }
...
ggml_tensor * llama_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    if (use_paged && sinfo.n_stream() == 1 && !sinfo.strm.empty()) {
        const uint32_t seq_id = sinfo.strm[0];
        ggml_tensor* v_from_pool = paged_cache.gather_v_view(ctx, nullptr, seq_id, n_kv);
        if (v_from_pool) {
            return v_from_pool;
        }
        // Fallback: if gather fails, try original path
...
ggml_tensor * llama_kv_cache::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    // Paged mode: ALSO write to BlockPool (real data source)
    // Keep original write for compatibility checks
    if (use_paged && !sinfo.idxs.empty()) {
        const uint32_t seq_id = sinfo.strm.empty() ? 0 : sinfo.strm[0];
        const int n_tokens = (int)k_cur->ne[2];
        // Write to BlockPool as the real data source
        paged_cache.write_tokens(seq_id, k_cur, nullptr, n_tokens);
    }

    // Still write to original layer.k/v for compatibility (the "傀儡")
    const int32_t ikv = map_layer_ids.at(il);
...
ggml_tensor * llama_kv_cache::cpy_v(ggml_context * ctx, ggml_tensor * v_cur, ggml_tensor * v_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    // Paged mode: ALSO write to BlockPool
    if (use_paged && !sinfo.idxs.empty()) {
        const uint32_t seq_id = sinfo.strm.empty() ? 0 : sinfo.strm[0];
        const int n_tokens = (int)v_cur->ne[2];
        paged_cache.write_tokens(seq_id, nullptr, v_cur, n_tokens);
    }
...
void llama_kv_cache::register_prefix(llama_seq_id seq_id, const int32_t* tokens, int n_tokens) {
    if (!use_paged) {
        return;
    }
    paged_cache.register_prefix(seq_id, tokens, n_tokens);
}
