...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        // Paged mode temporarily disabled - using dual-write approach
        // The dual-write approach (cpy_k/cpy_v writing to BlockPool) works reliably
        // and provides paged-like benefits without the graph-building complexity
        // TODO: Re-enable when graph building with minimal k/v is resolved

        if (v_trans) {
            v = ggml_transpose(ctx0, v);
        }

        // Cast to F16 for flash attention (same as normal path)
        ...
        // Always use normal path - paged inference happens via get_k/get_v at runtime
