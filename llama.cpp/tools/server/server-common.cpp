    ...
    std::vector<raw_buffer> & out_files)
{
    ...
    if (body.contains("response_format")) {
        ...
        // 2026-05-02: [TuriX Support] When response_format is provided with json_schema or json_object,
        // disable thinking to ensure JSON output goes to content field instead of reasoning_content
        // This enables TuriX to work with structured output
        if (response_type == "json_schema" || response_type == "json_object") {
            // Set chat_template_kwargs with enable_thinking=false to disable thinking
            json chat_template_kwargs = json_value(body, "chat_template_kwargs", json::object());
            chat_template_kwargs["enable_thinking"] = false;
            body["chat_template_kwargs"] = chat_template_kwargs;
            
            // Also set reasoning_format in the body so it gets propagated
            body["reasoning_format"] = "none";
            
            // Add grammar to force plain JSON output without markdown wrapping
            if (!json_schema.is_null()) {
                try {
                    std::string grammar_str = json_schema_to_grammar(json_schema);
                    body["grammar"] = grammar_str;
                } catch (const std::exception & e) {
                    // If grammar conversion fails, continue without it
                }
            }
        }
    }
