...
extern "C" {
    ...
    // Paged KV Cache descriptor for warmup and dimension-only operations
    // This structure carries only logical dimensions, NOT actual data pointers
    // Used to build computation graphs without allocating physical KV cache
    struct ggml_paged_kv_desc {
        int32_t n_embd;      // embedding dimension per head
        int32_t n_head;     // number of heads
        int32_t n_ctx;      // logical context length
        int32_t n_tokens;   // current token count
        int32_t block_size; // page block size (e.g., 16)
    };
    ...
    GGML_API struct ggml_tensor * ggml_flash_attn_ext(
            ...
            float                 max_bias,
            float                 logit_softcap,
            const void           * paged_desc);  // optional: paged KV descriptor for dimension-only mode
