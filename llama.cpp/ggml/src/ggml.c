...
struct ggml_tensor * ggml_flash_attn_ext(
        ...
        float                 max_bias,
        float                 logit_softcap,
        const void           * paged_desc) {

    // Paged mode: when paged_desc is provided, use it for dimension-only verification
    // This allows warmup without allocating physical KV cache
    if (paged_desc) {
        const struct ggml_paged_kv_desc * desc = (const struct ggml_paged_kv_desc *)paged_desc;

        // In paged mode, skip dimension checks - k/v may be uninitialized
        // The actual dimensions will be validated during inference from BlockPool
        int64_t ne[4] = { (int64_t)desc->n_embd, 1, (int64_t)desc->n_head, 1 };
        struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
        ...
        result->src[0] = q;
        result->src[1] = k;  // May be NULL in paged mode - resolved at runtime
        result->src[2] = v;  // May be NULL in paged mode - resolved at runtime
        result->src[3] = mask;

        return result;
    }

    // Original path: normal inference with physical k/v tensors
